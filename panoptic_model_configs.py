...
parent_dir_level2 = os.path.dirname(os.path.dirname(CONFIGPATH))
...
class MuTILsParams(object):
    ...
    #     'data'
    # )
    root = opj(parent_dir_level2, "MuTILs_Panoptic", "data")
    ...
    roi_unet_params.update({
        ...
        'diff_mode': 'raw',  # keep ROI stable for HPF proposal
    })
    hpf_unet_params = deepcopy(base_unet_params)
    hpf_unet_params.update({
        'in_channels_left': 3,  # rgb
        'in_channels_right': 3,  # placeholder; updated at runtime
        'depth': 5,  # editable param
        'diff_mode': 'qkv',  # enable qkv only in HPF branch
    })
    # all params
