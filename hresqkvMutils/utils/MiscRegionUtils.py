...
#     load_configs, reverse_dict, _div, abserr
# )
# from mutils_panoptic.MuTILs import MuTILs
# from utils.TorchUtils import load_torch_model, t2np
# from configs.panoptic_model_configs import MuTILsParams
# from utils.torchvision_transforms import PILToTensor
...
def get_configured_logger(logdir: str, prefix="", toscreen=True, tofile=True):
    """
    配置日志记录器，支持将日志输出到屏幕和/或文件。

    参数:
    - logdir (str): 日志文件的目录路径。
    - prefix (str): 日志文件名前缀。
    - toscreen (bool): 是否将日志输出到屏幕。
    - tofile (bool): 是否将日志输出到文件。
    ...
    import logging
    import datetime
    logger = logging.getLogger("MuTILsWSIRunner")
    ...
    if tofile:
        os.makedirs(logdir, exist_ok=True)  # 创建目录（如果不存在）

        now = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S.%f")
        logfile = os.path.join(logdir, f'{prefix}_{now}.log')
