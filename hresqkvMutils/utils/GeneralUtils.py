...
def append_row_to_df_or_create_it(where: str, df: pd.DataFrame):
    """Appends a row to a DataFrame stored in a CSV file, or creates it if it doesn't exist."""
    if not os.path.exists(where):
        df.to_csv(where, index=True, header=True)
    else:
        existing_df = pd.read_csv(where, index_col=0)
        
        # 获取要添加的 df 的 index 和 existing_df 的 index
        new_index = df.index
        existing_index = existing_df.index
        
        # 检查要添加的 index 是否在 existing_df 中
        if any(index in existing_index for index in new_index):
            
             # 如果索引重复，尝试合并列
            df = _merge_columns(existing_df, df)
            df.to_csv(where, index=True,header=True)
