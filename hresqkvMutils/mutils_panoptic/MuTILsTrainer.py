...
parser.add_argument('-f', type=int, default=5, help='fold')
...
#     # 将 'slide' 列转换为字符串类型
#     all_stats.loc[:, 'slide'] = all_stats.loc[:, 'roiname'].map(test_loader.dataset._r2s).astype(str)
#
...
def evaluate_mutils(
    ...
    for batchdata, truth in test_loader:
        ...
        # # 打印关键信息，查看维度是否与预期相符
        # print("roi_saliency_matrix shape:", inference['roi_saliency_matrix'].shape)
        # print("roi_region_logits shape:", inference['roi_region_logits'].shape)
        # print("hpf_region_logits shape:", inference['hpf_region_logits'].shape)
    ...
    # add meta
    # 将 'slide' 列转换为字符串类型以避免 FutureWarning
    all_stats.loc[:, 'slide'] = all_stats.loc[:, 'roiname'].map(test_loader.dataset._r2s).astype(str)
    ...
    # 获取数值类型的列
    numeric_cols = agg_stats.select_dtypes(include=[np.number]).columns

    # get aggregate stats over whole set -- mean
    stdf_mean = agg_stats[numeric_cols].mean(axis=0).to_frame().T  # this ignores nans.

    # get aggregate stats over whole set -- std
    stdf_std = agg_stats[numeric_cols].std(axis=0).to_frame().T  # this ignores nans.
    stdf_std.loc[:, 'epoch'] = epoch
...
def _train_one_mutils_epoch(
        ...
        savedir: str, ckpt_path: str, scaler: GradScaler, use_amp: bool):
    ...
    for batchdata, truth in train_loader:
        ...
        # run in forward mode (train) with AMP
        ...
        if not torch.isfinite(loss_dict['all']):
            print(
                f"Epoch {epoch} batch {batchidx}: non-finite loss detected; "
                f"skip optimizer step for this batch."
            )
            continue
        ...
        # backpropagate (AMP-aware path on CUDA, fp32 fallback on CPU)
        if use_amp:
...
def train_mutils_fold(
    ...
    # Pure float16 training is unstable here (no AMP GradScaler path in this trainer).
    # Force fp32 data tensors unless you explicitly add AMP+scaler support.
    if mtp.train_dataset_params.get('float16', False) or mtp.test_dataset_params.get('float16', False):
        print("[MuTILsTrainer] float16 dataset path detected; force using float32 for stability.")
    mtp.train_dataset_params['float16'] = False
    mtp.test_dataset_params['float16'] = False
    ...
    # 检查 mtp.train_loader_kwa 中是否有 num_workers 参数
    if 'num_workers' in mtp.train_loader_kwa:
        del mtp.train_loader_kwa['num_workers']


    train_loader = DataLoader(dataset=train_dataset, **mtp.train_loader_kwa, num_workers=0)
    n_batches = len(train_loader)
    assert smooth_window <= n_batches

    if 'num_workers' in mtp.test_loader_kwa:
        del mtp.test_loader_kwa['num_workers']

    # Testing dataset loader
    test_dataset = MuTILsDataset(
        root=mtp.root, slides=test_slides, **mtp.test_dataset_params)
    test_loader = DataLoader(dataset=test_dataset, **mtp.test_loader_kwa, num_workers=0)
...
if __name__ == '__main__':
    ...
    # from utils.GeneralUtils import save_configs
    from pathlib import Path

    # paths
    #BASEPATH = opj(os.path.expanduser('~'), 'Desktop', 'cTME')
    CURRENT_DIR = Path(__file__).resolve().parent
    BASEPATH = CURRENT_DIR.parent
    # BASEPATH = r'D:\文件\研究生\陈老师的工作\MuTILs_Panoptic'
    model_name = 'mutils_02252026'
    model_root = opj(BASEPATH, 'results', 'mutils', 'models', model_name)
    maybe_mkdir(model_root)

    # load configs
    configs_path = opj(model_root, 'panoptic_model_configs.py')
    ...
    #     save_configs(
    #         configs_path=opj(
    #             BASEPATH, 'ctme', 'configs', 'panoptic_model_configs.py'),
