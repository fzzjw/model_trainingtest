...
# from utils.TorchUtils import transform_dlinput
# import utils.torchvision_transforms as tvdt
# from configs.panoptic_model_configs import RegionCellCombination
...
class MuTILsDataset(object):
    ...
    def _set_roiweights_training(self):
        ...
        for cls, clswt in self.region_weights.items():
            # 转换相关列为 float 类型
            tmp_rs[cls] = tmp_rs[cls].astype(np.float64) * clswt.astype(np.float64)
        tmp_rs = tmp_rs.sum(1)
...
if __name__ == '__main__':
    ...
    from configs.panoptic_model_configs import VisConfigs
