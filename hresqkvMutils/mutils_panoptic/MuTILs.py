...
from MuTILs_Panoptic.mutils_panoptic.dual_diff_refine_unet import DualDiffRefineUNet
...
class MuTILs(nn.Module):
    ...
    def __init__(
        ...
        if self.roi_unet_params is None:
            ...
            self.roi_unet_params.update({
                'in_channels_left': 3,  # rgb
                'in_channels_right': 3,  # rgb
                'depth': 5,  # default: 5
            })
        if 'in_channels' in self.roi_unet_params:
            self.roi_unet_params.setdefault(
                'in_channels_left', self.roi_unet_params['in_channels'])
            self.roi_unet_params.setdefault(
                'in_channels_right', self.roi_unet_params['in_channels'])
            self.roi_unet_params.pop('in_channels', None)
        self.roi_unet_params.update({
        ...
        # hpf nuclei model params (need depth for sizing)
        if self.hpf_unet_params is None:
            self.hpf_unet_params = deepcopy(unet_params)
            self.hpf_unet_params.update({
                'in_channels_left': 3,  # rgb
                'in_channels_right': 3,  # placeholder; updated after richannels
                'depth': 5,  # default: 5
            })
        if 'in_channels' in self.hpf_unet_params:
            self.hpf_unet_params.setdefault(
                'in_channels_left', self.hpf_unet_params['in_channels'])
            self.hpf_unet_params.setdefault(
                'in_channels_right', self.hpf_unet_params['in_channels'])
            self.hpf_unet_params.pop('in_channels', None)
        ...
        self.hpf_unet_params.update({
            ...
            'padding': True,  # must be true!
            'in_channels_left': 3,
            'in_channels_right': self.richannels,
        })
        self.roi_unet = DualDiffRefineUNet(**self.roi_unet_params)
        self.hpf_unet = DualDiffRefineUNet(**self.hpf_unet_params)
    ...
    def process_hpfs(self, hres_rgb, roi_logits, roi_intermed, hpf_bounds):
        ...
        # resize roi intermed to match hpf input
        if hpf_roi_intermed.shape[2:] != hpf_rgbs.shape[2:]:
            hpf_roi_intermed = F.interpolate(
                hpf_roi_intermed,
                size=hpf_rgbs.shape[2:],
                mode='bilinear',
                align_corners=False,
            )

        # get hpf nucleus predictions
        hpf_nuclei_pre = self.hpf_unet(hpf_rgbs, hpf_roi_intermed)
